...
function _M:before()
    ...
    self.validate:load(data)
    self.param = data
    if not self.validate:validate(self.action) then
        return self:fail(self.validate.err)
    end
    return true
end
...
--页面跳转
function _M:outJson(data)
    if not next(data) then
        self:showError("返回数据错误")
    end
    local util = require("vendor.Util")
    ngx.header["Content-Type"] = "application/json; charset=utf-8"
    ngx.say(util.json_encode(data))
    ngx.status = 200
end

function _M:success(data, message, code)
    local return_data = {}
    if data and next(data) then
        return_data.data = data
    end
    return_data.message = message or "success"
    return_data.code = code or 0
    return self:outJson(return_data)
end

function _M:fail(message, code)
    message = message or "fail"
    code = code or 1
    return self:outJson({ code = code, message = message })
end
