...
--根据规则校验参数
function _M:validate(action)
    local rules = self:rules()
    rules = rules[action] or {}
...
--检查参数是否为空
function _M:length(key, rule)
    log:log(key)
    log:log(rule)
    key = type(key) == 'table' and key or { key }
    for _, k in ipairs(key) do
        if self.err[k] == nil and ((rule.min and string.len(self[k]) < rule.min) or (rule.max and string.len(self[k]) > rule.max)) then
            local msg = rule.message or key .. lang.length
            self:addError(k, msg)
        end
    end
end
